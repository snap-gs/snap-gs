...
main () {
	...
	for k in ${!SNAPGS_INSTALL_LOBBIES[@]}; do
		...
		sudo -u snap-gs ln -s -f -T ../flags/forcestart /opt/snap-gs/SnapshotVR/$i/spec/forcestart
		sudo -u snap-gs ln -s -f -T ../flags/forcestop /opt/snap-gs/SnapshotVR/$i/spec/forcestop
		...
		if [[ $n != 1 ]]; then
			...
			sudo -u snap-gs ln -s -f -T ../peer /opt/snap-gs/SnapshotVR/$i/spec/peer
			sudo -u snap-gs ln -s -f -T ../$j/self /opt/snap-gs/SnapshotVR/$i/peer
			sudo -u snap-gs ln -s -f -T stat /opt/snap-gs/SnapshotVR/$i/self
		fi

		for x in /opt/snap-gs/SnapshotVR/$i/flags/{forcestart,forcestop,start,session,password}; do
			[[ -e $x ]] || sudo -u snap-gs touch $x
			sudo -u snap-gs chmod 666 $x
		done
		if [[ $i == 1 && $SNAPGS_LOBBY_SESSION != test\ * ]] && [[ ! $SNAPGS_LOBBY_PASSWORD || $n != 1 ]]; then
			sudo -u snap-gs touch -a /opt/snap-gs/SnapshotVR/$i/flags/up
		else
			sudo -u snap-gs rm -f /opt/snap-gs/SnapshotVR/$i/flags/up
		fi
	...
	for i in ${SNAPGS_INSTALL_LOBBIES[@]}; do
		...
		if [[ -e /opt/snap-gs/SnapshotVR/$i/stat/up ]]; then
			sudo -u snap-gs bash -c "echo > /opt/snap-gs/SnapshotVR/$i/spec/start"
		fi
	done
